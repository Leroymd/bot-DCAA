...
import { AlertCircle, Calendar, DollarSign, TrendingUp, Percent } from 'lucide-react';
...
const Stats = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const performanceResponse = await axios.get('/api/performance/data');
        ...
        const pnlResponse = await axios.get(`/api/performance/pnl?days=${days}`);
        setPnlData(pnlResponse.data);
        
        // Получение истории сделок
        const tradesResponse = await axios.get('/api/performance/trades?limit=20');
  ...
  const calculateStats = () => {
    if (!performance || performance.length === 0 || !tradeHistory || tradeHistory.length === 0) {
      return {
        winRate: 0,
        totalTrades: 0,
        avgProfit: 0,
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-gray-300">Загрузка статистики...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-gray-800 p-4 rounded-lg">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm text-gray-400">Средняя прибыль</p>
                <h3 className="text-2xl font-bold mt-1">{stats.avgProfit.toFixed(2)}%</h3>
              </div>
              <div className="bg-gray-700 p-2 rounded-full">
